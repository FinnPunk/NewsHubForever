...
const createTouchTexture = () => {
  ...
  const drawPoint = p => {
    const posX = p.x * size;
    const posY = (1 - p.y) * size;
    const ageRatio = p.age / maxAge;
    let intensity;
    if (ageRatio < 0.3) {
      intensity = Math.sin((ageRatio / 0.3 * Math.PI) / 2);
    } else {
      const t = 1 - (ageRatio - 0.3) / 0.7;
      intensity = -t * (t - 2) || 0;
    }
    intensity *= p.force;
    ...
    ctx.arc(posX - offset, posY - offset, radius, 0, 6.283185307179586);
    ctx.fill();
  };
  const addTouch = norm => {
    let force = 0;
...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (!mustReinit && prevConfigRef.current) {
      mustReinit = prevConfigRef.current.antialias !== cfg.antialias ||
    ...
    if (mustReinit) {
      ...
      const setSize = () => {
        ...
        uniforms.uResolution.value.set(canvasW, canvasH);
        if (threeRef.current?.composer) {
          threeRef.current.composer.setSize(canvasW, canvasH);
        }
      ...
      if (noiseAmount > 0) {
        ...
        const noiseEffect = new Effect(
          'NoiseEffect',
          `uniform float uTime; uniform float uAmount; float hash(vec2 p){ return fract(sin(dot(p, vec2(127.1,311.7))) * 43758.5453);} void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){ float n=hash(floor(uv*vec2(1920.0,1080.0))+floor(uTime*60.0)); float g=(n-0.5)*uAmount; outputColor=inputColor+vec4(vec3(g),0.0);} `,
          {
            uniforms: new Map([
              ['uTime', new THREE.Uniform(0)],
      ...
      const animate = () => {
        ...
        if (liquidEffect) liquidEffect.uniforms.get('uTime').value = currentTime;
        if (composer) {
